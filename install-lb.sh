...
sudo virt-customize -a $IMDIR/${CLUSTER_NAME}-lb.qcow2 \
  --uninstall cloud-init \
  --ssh-inject root:file:$SSH_KEY --selinux-relabel \
  --sm-credentials "${RHNUSER}:password:${RHNPASS}" --sm-register \
  --install haproxy

sudo virt-install --import --name ${CLUSTER_NAME}-lb \
  --disk $IMDIR/${CLUSTER_NAME}-lb.qcow2 --memory 548 --cpu host --vcpus 1 \
  --network network=${CLUSTER_NAME},model=virtio,mac=$CLUSTER_MAC:03 \
  --noreboot --noautoconsole

sudo virsh start ${CLUSTER_NAME}-lb

while [[ $(sudo virsh domstate ${CLUSTER_NAME}-lb) != "running" ]]; do sleep 5; done

ssh-keygen -R lb.${CLUSTER_NAME}.${BASE_DOM}
ssh-keygen -R $CLUSTER_SUBNET_BASE.3
ssh -o StrictHostKeyChecking=no root@lb.${CLUSTER_NAME}.${BASE_DOM} true

ssh root@lb.${CLUSTER_NAME}.${BASE_DOM} <<EOF
